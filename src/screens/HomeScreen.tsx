...
// ─────────────────────────────────────────────
//  FLOATING ORBS  — ambient energy background
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
//  FLOATING ICONS  — fitness icons that breathe + drift
// ─────────────────────────────────────────────
function FloatingBackground({ scrollY }: { scrollY: SharedValue<number> }) {
  ...
  useEffect(() => {
    ...
    y1.value = withRepeat(
      withSequence(
        withTiming(-14, { duration: 3200, easing: Easing.inOut(Easing.quad) }),
        withTiming(6, { duration: 2800, easing: Easing.inOut(Easing.quad) })
      ),
      -1,
      true
    );
    rot1.value = withRepeat(
      withSequence(
        withTiming(8, { duration: 4000, easing: Easing.inOut(Easing.quad) }),
        withTiming(-6, { duration: 3500, easing: Easing.inOut(Easing.quad) })
      ),
      -1,
      true
    );
    ...
    y2.value = withDelay(
      ...
      withRepeat(
        withSequence(
          withTiming(-22, { duration: 2500, easing: Easing.inOut(Easing.sin) }),
    ...
    rot2.value = withDelay(
      ...
      withRepeat(
        withSequence(
          withTiming(-12, { duration: 3800, easing: Easing.inOut(Easing.quad) }),
  ...
  const icon1Style = useAnimatedStyle(() => ({
    transform: [
      { translateY: y1.value + interpolate(scrollY.value, [0, 300], [0, -30]) },
  ...
  const icon2Style = useAnimatedStyle(() => ({
    transform: [
      { translateY: y2.value + interpolate(scrollY.value, [0, 300], [0, -50]) },
  ...
  const icon3Style = useAnimatedStyle(() => ({
    transform: [
      { translateY: y3.value + interpolate(scrollY.value, [0, 300], [0, -20]) },
    ],
  }));
  ...
  return (
    <>
      ...
      <Reanimated.View style={[styles.bgIconWrap, styles.bgIcon1, icon1Style]}>
        <MaterialCommunityIcons name="dumbbell" size={88} color="rgba(196,181,253,0.09)" />
      </Reanimated.View>
      <Reanimated.View style={[styles.bgIconWrap, styles.bgIcon2, icon2Style]}>
        <MaterialCommunityIcons name="run-fast" size={96} color="rgba(167,139,250,0.12)" />
      </Reanimated.View>
      <Reanimated.View style={[styles.bgIconWrap, styles.bgIcon3, icon3Style]}>
...
// ─────────────────────────────────────────────
//  HERO GLOW  — pulsing radial light (FIXED: outside BlurView)
// ─────────────────────────────────────────────
function HeroGlow() {
  const scale = useSharedValue(1);
  ...
  return (
    <Reanimated.View style={[styles.heroGlowOuter, style]} pointerEvents="none">
      <LinearGradient
        colors={["rgba(124,58,237,0.85)", "rgba(99,102,241,0.4)", "transparent"]}
...
// ─────────────────────────────────────────────
//  SHIMMER LINE  — sliding accent across hero
// ─────────────────────────────────────────────
function ShimmerLine() {
  ...
  return (
    <Reanimated.View style={[styles.shimmerWrap, style]} pointerEvents="none">
      <LinearGradient
        colors={["transparent", "rgba(255,255,255,0.12)", "rgba(255,255,255,0.04)", "transparent"]}
        start={{ x: 0, y: 0 }}
        end={{ x: 1, y: 0 }}
...
// ─────────────────────────────────────────────
//  ANIMATED COUNT TEXT  — number counts up on enter
// ─────────────────────────────────────────────
...
function CountUpText({ value, style }: { value: number; style: any }) {
  ...
  // Use JS-side polling for display (clean & works in Reanimated 4)
  useEffect(() => {
...
// ─────────────────────────────────────────────
//  METRIC CARD  — with count-up + animated border glow
// ─────────────────────────────────────────────
function MetricCard({
  icon,
  label,
...
// ─────────────────────────────────────────────
//  WORKOUT CARD  — staggered entrance + spring press
// ─────────────────────────────────────────────
function WorkoutCard({
  ...
  const accent = categoryColors[item.category] ?? categoryColors.default;
...
// ─────────────────────────────────────────────
//  ADD WORKOUT FAB  — spring bounce on mount
// ─────────────────────────────────────────────
function FABButton({ onPress }: { onPress: () => void }) {
  const scale = useSharedValue(0);

  useEffect(() => {
    scale.value = withDelay(600, withSpring(1, { damping: 12, stiffness: 200 }));
  }, []);
  ...
  const handleDelete = (id: number) => {
    Alert.alert("Delete Workout", "Remove this session?", [
      { text: "Cancel", style: "cancel" },
      {
        ...
        style: "destructive",
        onPress: () => {
          try { deleteWorkout(id); loadWorkouts(); } catch (e) { console.log(e); }
        },
      },
    ]);
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea} edges={["top"]}>
      <View style={styles.container}>
          ...
          ListHeaderComponent={
            <>
              ...
              <Reanimated.View
                ...
                style={[styles.heroOuter, heroAnimatedStyle]}
              >
                {/* Glow lives OUTSIDE BlurView so it's actually visible */}
                <HeroGlow />
                  ...
                  <View style={styles.heroTopRow}>
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={styles.heroSubtitle}>
                        Build strength, track performance, and make your routine feel elite.
                      </Text>
                    </View>
                    ...
                    <View style={styles.progressBarBg}>
                      <Reanimated.View
                        ...
                        style={[
                          ...
                          { width: `${Math.min((totalWorkouts % 5) * 20, 100)}%` },
                        ]}
                      />
                    </View>
            ...
            <Reanimated.View entering={FadeInDown.delay(300).duration(500)}>
              <BlurView intensity={24} tint="dark" style={styles.emptyBox}>
                ...
                <Text style={styles.emptySubtitle}>
                  Add your first session and start building momentum.
                </Text>
              </BlurView>
            </Reanimated.View>
          }
        ...
        {/* ── FAB ── */}
        <FABButton onPress={() => navigation.navigate("AddWorkout" as any)} />
      </View>
    </SafeAreaView>
  );
}
...
const styles = StyleSheet.create({
  ...
  bgIcon1: { top: 80, left: -10, opacity: 1 },
  bgIcon2: { top: 200, right: -18, opacity: 1 },
  bgIcon3: { top: 380, left: 8, opacity: 1 },
  ...
  heroOuter: {
    ...
    marginBottom: 18,
    borderRadius: 30,
    overflow: "visible", // ← key: lets glow show outside
  },
  heroGlowOuter: {
    position: "absolute",
    width: 320,
